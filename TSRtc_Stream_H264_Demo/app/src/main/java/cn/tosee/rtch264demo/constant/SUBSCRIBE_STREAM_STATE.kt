package cn.tosee.rtch264demo.constant
...
object VIDEO_STREAM_TYPE {
    const val VIDEO_STREAM_FRAME = 1  //1：原始流
    const val VIDEO_STREAM_H264 = 2   //2：h264裸流
    const val VIDEO_STREAM_CUSTOM = 3 //3：未知的用户自定义类型
};
object VIDEO_PIXEL_FORMAT {
    const val VIDEO_PIXEL_UNAVILABLE = -1 //-1：无效接口
    const val VIDEO_PIXEL_I420 = 0        //0：I420 格式
    const val VIDEO_PIXEL_BGRA = 1        //1：BGRA 格式
    const val VIDEO_PIXEL_NV21 = 2        //2：NV21 格式
    const val VIDEO_PIXEL_RGBA = 3        //3：RGBA 格式
    const val VIDEO_PIXEL_IMC2 = 4        //4：IMC2 格式
    const val VIDEO_PIXEL_ARGB = 5        //5：ARGB 格式
    const val VIDEO_PIXEL_NV12 = 6        //6：NV12 格式
    const val VIDEO_PIXEL_I422 = 7        //7：I422 格式
    const val VIDEO_PIXEL_COUNT = 8
};
//频道场景
