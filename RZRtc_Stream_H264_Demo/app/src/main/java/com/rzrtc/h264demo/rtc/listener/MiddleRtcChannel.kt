...
class MiddleRtcChannel(val rtcChannel: IRtcChannel) {
    ...
    fun createVideoStream(
        streamName: String?, videoSource: IVideoSource?, type:Int, event: IRtcStreamEventHandler?,
        videoCfg: VideoEncoderConfiguration?
    ): IRtcStream {
        val result = rtcChannel.createVideoStream(streamName, videoSource, type, event, videoCfg)
